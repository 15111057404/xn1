<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>

	<head>
	  	<link rel="stylesheet" href="/xiaoneinews/css/css/common.css" type="text/css"></link>
		<script type="text/javascript">
  <!-- 
function changeBG(o){
	o.style.backgroundColor="#5C75AA";
}
function changeBG2(o){
	o.style.backgroundColor="";
}
-->
  </script>

	</head>

		<div class="head">
			<ul class="home_navi">
			<li onmouseover="changeBG(this)" onmouseout="changeBG2(this)"><a class="top" href="/xiaoneinews/exit.htm">首页</a></li>
			<li onmouseover="changeBG(this)" onmouseout="changeBG2(this)"><a class="top" href="/xiaoneinews/self/homePage.jsp">个人主页</a></li>
			<li>搜索</li>
			<li style="width: 170px;"><input type="text" /><img src="/xiaoneinews/images/front/sou.jpg" /></li>
			</ul>
			
			<ul class="navi">
				<li onmouseover="changeBG(this)" onmouseout="changeBG2(this)">
					<a class="anavi" href="#">账号</a>
				</li>
				<li onmouseover="changeBG(this)" onmouseout="changeBG2(this)">
					<a class="anavi" href="#">隐私</a>
				</li>
				<li onmouseover="changeBG(this)" onmouseout="changeBG2(this)">
					<a  class="anavi" href="exit.htm">退出</a>
				</li>
			</ul>
		</div>

